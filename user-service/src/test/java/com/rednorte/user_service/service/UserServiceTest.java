    ...
    class UserServiceTest {
        ...
        class GetDoctors {
            ...
            void shouldReturnOnlyActiveDoctors() {
                User doctor = buildUser(UserRole.DOCTOR);
                User inactiveDoctor = buildUser(UserRole.DOCTOR);
                inactiveDoctor.setActive(false);
                User admin = buildUser(UserRole.ADMIN);

                when(repository.findAll()).thenReturn(List.of(doctor, inactiveDoctor, admin));

                List<UserResponseDTO> result = service.getDoctors();
            ...
            @Test
            @DisplayName("Retorna lista vacía si no hay doctores activos")
            void shouldReturnEmptyWhenNoDoctors() {
                when(repository.findAll()).thenReturn(List.of(buildUser(UserRole.ADMIN)));

                assertThat(service.getDoctors()).isEmpty();
            }
        }
    }